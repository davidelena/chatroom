...
func (this *SmsProcessor) SendSmsPrivateMes(receiverId int, content string) (err error) {
	var mes message.Message
	mes.Type = message.SmsMesPrivateType

	var smsMes message.SmsMes
	smsMes.UserId = CurrentUser.UserId
	smsMes.ReceiverId = receiverId
	smsMes.Content = content
	smsMes.Status = message.UserOnline

	data, err := json.Marshal(smsMes)
	if err != nil {
		fmt.Println("SendSmsPrivateMes json.Marshal err=", err)
		return err
	}
